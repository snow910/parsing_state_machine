...
TEST_CASE( "ReparseRule test", "[ReparseRule][psm]" )
{
	// one reparse level
	{
		...
		CHECK( p.parse( "012*35678*9*" ) == ParsingResult{ ParsingStatus::Success, "012*35678*9*"sv } );
	...
	}
	{
		...
		CHECK( p.parse( str, true ) == ParsingResult{ ParsingStatus::Success, "012*35678*9*"sv } );
		CHECK( p.actionFunction().n == 6 );
	}
	...
	// reparse level, then sequence level, then second reparse level
	{
		...
		CHECK( p.parse( "012abcABC*35678defDEF*9ghiGHI*" ) == ParsingResult{ ParsingStatus::Success, "012abcABC*35678defDEF*9ghiGHI*"sv } );
